        ...
        private void CreaGriglia(IEnumerable<string> articoliNonCompatibiliOrdine, List<string> outOfStockItems)
        {
            ...
            if (_STOCK != null && _STOCK.Any())
            {
                foreach (var _i in _STOCK) 
                {
                    ...
                    _h = _h + "<div class=\"col-4 col-md-4 text-end check-pos\">" + _i.QTYSTU_0.ToString("0.##") + "&nbsp;&nbsp;</div>";
        ...
        protected void btn_Conferma_Click(object sender, EventArgs e)
        {
            ...
            using (SqlConnection conn = new SqlConnection(connectionSQL))
            {
                ...
                try
                {
                    foreach (var item in _STOCK)
                    {
                        var QTY = item.QTYSTU_0;

                        foreach (Obj_YTSORDAPE _ord in _SQL.Obj_YTSORDAPE_Ordini(_USR.FCY_0, _BPCORD, _BPAADD, _DATE_DA, _DATE_A, item.ITMREF_0, item.STU_0))
                        {
                            ...
                            if (_ord.QTY_MANC > 0 && QTY > 0)
                            {
                                ...
                                {
                                    bool ok = cls_TermWS.WS_AllocaDett(_ord.SOHNUM_0, _ord.SOPLIN_0.ToString(), _ord.SOQSEQ_0.ToString(), _USR.FCY_0, item.LOT_0, item.PALNUM_0, Properties.Settings.Default.SPED_Ubic, _QTY_PREP, out err);
